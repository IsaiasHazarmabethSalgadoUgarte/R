...
hotegfb<-function(s1,s2,p1,p2,n1,n2,cl){
  ...
  if(t2[1,1]>t2t){
    llLi = dp[1,1]-(((n1+n2)/(n1*n2))*s[1,1]^.5)^.5
    ulLi = dp[1,1]+(((n1+n2)/(n1*n2))*s[1,1]^.5)^.5
    fcritLi= (n1*n2*(n1+n2-4)*(dp[1,1])^2)/(3*(n1+n2)*(n1+n2-2)*s[1,1])
    
    llk = dp[2,1]-(((n1+n2)/(n1*n2))*s[2,2]^.5)^.5
    ulk = dp[2,1]+(((n1+n2)/(n1*n2))*s[2,2]^.5)^.5
    Fcritk= (n1*n2*(n1+n2-4)*(dp[2,1])^2)/(3*(n1+n2)*(n1+n2-2)*s[2,2])
    
    llto = dp[3,1]-(((n1+n2)/(n1*n2))*s[3,3]^.5)^.5
    ulto = dp[3,1]+(((n1+n2)/(n1*n2))*s[3,3]^.5)^.5
    Fcritto= (n1*n2*(n1+n2-4)*(dp[3,1])^2)/(3*(n1+n2)*(n1+n2-2)*s[3,3])
    
    con<-cl
    cat("        Bonferroni C.I. of ",con,"%\t\t\t  Critical F\n")
